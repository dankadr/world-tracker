...
export default function App() {
  ...
  const { routeMode, routePoints, toggleRouteMode, addPoint: addRoutePoint, clearRoute, totalDistanceKm } = useRouteDrawing(userId);

  // Collect all visited region keys across all trackers for exploration depth.
  // Keys: anonymous → "swiss-tracker-visited-{id}", auth'd → "n{userId}-visited-{id}"
  // Must use secureStorage.getItemSync so encrypted values are decrypted correctly.
  const allVisitedKeys = useMemo(() => {
    ...
    try {
      for (let i = 0; i < localStorage.length; i++) {
        ...
        if (!key) continue;
        const match = key.match(/^(?:swiss-tracker(?:-u\d+)?|n\d+)-visited-(\w+)/);
        if (match) {
