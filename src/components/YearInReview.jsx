...
export default function YearInReview({ year, onClose, embedded = false }) {
  const { user } = useAuth();
  ...
  if (!stats.hasData) {
    const emptyContent = (
      <div className={`yir-container yir-empty${embedded ? ' yir-container-embedded' : ''}`} onClick={e => !embedded && e.stopPropagation()}>
    ...
    if (embedded) return <div className="yir-embedded-wrap">{emptyContent}</div>;
    return createPortal(
  ...
  const mainContent = (
    <div
      className={`yir-container${embedded ? ' yir-container-embedded' : ''}`}
      ref={containerRef}
  ...
  const overlayContent = embedded ? (
    <div className="yir-embedded-wrap">{mainContent}</div>
  ) : createPortal(
    <div className="yir-overlay" onClick={onClose}>
      {mainContent}
    </div>,
